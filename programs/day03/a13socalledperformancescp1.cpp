...
perf stats  -e LLC-loads,LLC-load-misses yourExecutable

perf stat -B -e cache-references,cache-misses,cycles,instructions,branches,faults,migrations ./hello
...
Context switches occur when the operating system switches the execution context from one process or thread to another.
Interpretation: High context switch rates may indicate that the system is spending a significant amount of time switching between different tasks,
 which could impact overall performance. Excessive context switches can lead to increased overhead and decreased throughput.

CPU Migrations:
 CPU migrations occur when a thread is moved from one CPU core to another.
Interpretation: High CPU migration rates may indicate that the workload is not being efficiently scheduled across CPU cores. This can result in increased cache misses and synchronization overhead, leading to suboptimal performance.
Page Faults:

Definition: Page faults occur when a process accesses a memory page that is not currently mapped in physical memory and triggers a page fault exception.
Interpretation: High page fault rates may indicate that the working set of the application exceeds the available physical memory, leading to frequent disk I/O operations for page swapping. This can significantly degrade performance due to increased latency.
Cycles:

Definition: Cycles represent the total number of CPU clock cycles elapsed during the execution of the monitored workload.
Interpretation: The number of cycles provides an indication of the total computational effort required to execute the workload. Higher cycle counts may indicate that the workload is CPU-bound and may benefit from optimization efforts to reduce computational complexity.
Instructions:

Definition: Instructions represent the total number of instructions executed by the CPU during the monitored workload.
Interpretation: The number of instructions provides insights into the code's execution path and complexity. Analyzing instruction counts alongside other metrics can help identify hotspots and potential optimization opportunities within the code.
Branches and Branch Misses:

Definition: Branches represent the total number of control flow branches executed by the CPU, while branch misses represent the number of mispredicted branches.
Interpretation: Branches are crucial for program flow control, and branch misses can significantly impact performance due to pipeline stalls. 
High branch miss rates may indicate suboptimal branch prediction, which can lead to reduced CPU efficiency and lower instruction throughput.


timizing branch prediction in C++ code involves writing code that minimizes branch mispredictions and leverages the CPU's branch prediction mechanisms efficiently. Here are some techniques to optimize branch prediction:
...
int aabsoluteDifference(int a, int b) {
    int diff = a - b;
